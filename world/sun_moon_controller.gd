...
const AMBIENT_DAY:   float = 0.5 * 0.5
const AMBIENT_NIGHT: float = 0.5 * 0.2
...
func _apply_sun(total_seconds: float) -> void:
	...
	_env.ambient_light_energy = AMBIENT_DAY if sun_dir.y > 0.0 else AMBIENT_NIGHT
...
func _apply_moon(total_seconds: float) -> void:
	...
	moon.light_energy = clamp(moon_dir.y, 0.0, 1.0) * phase * 0.15
